...
class ${controller.name} : Service {
	...
	func ${route.name}(${route.parameters.map(parameter => `${parameter.name}: ${parameter.isArray ? "[" : ""}${this.typeMappings[parameter.type] || parameter.type}${parameter.isArray ? "]" : ""}`)}, completionHandler: @escaping (Error?, ${
		...
		let task = URLSession.shared.dataTask(with: request) { (data, response, error) in
			...
			do {
				...
				if res["data"] != nil {
					...
					return ${route.returnType.slice(0, route.returnType.length - 1).map(t => `result.map({ result in { return `)}${(() => {
						...
						if (type == "boolean") {
							...
							return `result == nil ? nil : JSONDecoder().decode(${type}, from: result)`;
						} 
					})()}${"})".repeat(route.returnType.length - 1)};
					
					completionHandler(nil, Double(result as! Double))
